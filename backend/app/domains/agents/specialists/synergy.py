...
class SynergyValueCreationAgent(BaseSpecialistAgent):
    ...
    async def _run_assessment(
        ...
        total_run_rate = sum(s.annual_run_rate_usd for s in synergies) if synergies else 4_500_000.0
        total_npv = sum(s.net_present_value_usd for s in synergies) if synergies else 14_200_000.0
        cost_syn = sum(s.annual_run_rate_usd for s in synergies if s.synergy_type == "COST") if synergies else 3_000_000.0
        ...
        findings = [
            GroundedFinding(
                ...
                detailed_reasoning=f"Identified ${total_run_rate:,.0f} run-rate synergies ({cost_pct:.0f}% cost, {rev_pct:.0f}% revenue) generating ${total_npv:,.0f} in incremental NPV.",
                severity_level="LOW",
        ...
        summary = (
            ...
            if synergies
            else "Standard M&A value creation waterfall modeled."
        )
